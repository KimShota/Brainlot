-- ========================================
-- SECURITY HARDENING - CORRECTED VERSION
...
DROP POLICY IF EXISTS "Allow read from study" ON storage.objects;
DROP POLICY IF EXISTS "Allow read from study bucket" ON storage.objects;
DROP POLICY IF EXISTS "Allow uploads to study" ON storage.objects;
DROP POLICY IF EXISTS "Allow uploads to study bucket" ON storage.objects;
...
DROP POLICY IF EXISTS "Users can upload to study" ON storage.objects;
...
DROP POLICY IF EXISTS "Users can delete mcqs for their files" ON mcqs;
DROP POLICY IF EXISTS "Users can insert mcqs for their files" ON mcqs;
DROP POLICY IF EXISTS "Users can update mcqs for their files" ON mcqs;
DROP POLICY IF EXISTS "Users can view mcqs for their files" ON mcqs;
...
DROP POLICY IF EXISTS "Users can delete jobs for their files" ON jobs;
DROP POLICY IF EXISTS "Users can insert jobs for their files" ON jobs;
DROP POLICY IF EXISTS "Users can update jobs for their files" ON jobs;
DROP POLICY IF EXISTS "Users can view jobs for their files" ON jobs;

-- Ensure users can only see their own jobs (via file ownership)
...
-- Ensure users can only insert jobs for their own files
...
BEGIN
    ...
    RAISE NOTICE 'Dangerous policies removed from subscription/usage tables';
END $$;
